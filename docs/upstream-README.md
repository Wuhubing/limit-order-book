# Limit Order Book — upstream README (archived)

Original README of the upstream repository
[brprojects/Limit-Order-Book](https://github.com/brprojects/Limit-Order-Book)
(MIT License, Benjamin Rienecker), pinned at commit `af6e534`. Archived here so
the project README can describe this repository's own contribution; the
upstream text is preserved verbatim below.
...
The average number of active limit orders for the test data is 10,000, and the average number of active stop or stop-limit orders is 1,000.
...
The next figure shows the mean latency for different order types that did not result in trades (i.e., not market or market limit orders). The error bars represent the 15th to 85th percentiles of orders. Canceling orders was the quickest, averaging 400ns, while modifying and adding orders took slightly longer, around 700ns.

The figure above shows how latency was affected by the number of trades resulting from a market or market limit order. A total of 845,242 trades were executed during the test.

Another key factor affecting latency was the number of AVL tree rebalances required after each order. Over the test, there were 252,504 AVL tree rebalances. The figure above shows that each tree rebalance significantly increased latency, averaging 2500ns per additional tree rebalance.
